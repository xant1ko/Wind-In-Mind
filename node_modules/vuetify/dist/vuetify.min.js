...
n.value=l?Math.min(n.value,t):t,a===t&&l===n.value||(y[e]=t,k())}}}const Ki=rt({items:{type:Array,default:()=>[]},renderless:Boolean,...Yi(),...it(),...el()},"VVirtualScroll"),qi=ft()({name:"VVirtualScroll",props:Ki(),setup(e,{slots:a}){const l=st("VVirtualScroll"),{dimensionStyles:n}=tl(e),{calculateVisibleItems:o,containerRef:r,markerRef:i,handleScroll:s,handleScrollend:u,handleItemResize:c,scrollToIndex:d,paddingTop:v,paddingBottom:p,computedItems:m}=Gi(e,t.toRef(()=>e.items))
...
return Ys(l,r+Math.floor(v*o.value),a.times.now)},getSlotScope:function(e){const t=Xs(e)
...
null!=t&&o({value:t}),C.value=!1,E.value=0},T=a=>{const o=P(a)
_.value=r(a),_.value&&(C.value=!0,_.value.contains(a.target)?E.value=Xc(a,_.value,e.direction):(E.value=0,null!=o&&n({value:o})),null!=o&&l({value:o}),t.nextTick(()=>_.value?.focus()))},D={passive:!0,capture:!0}
function B(e){const t=P(e)
null!=t&&n({value:t})}function F(e){e.stopPropagation(),e.preventDefault(),R(e),window.removeEventListener("mousemove",B,D),window.removeEventListener("mouseup",F)}function $(e){R(e),window.removeEventListener("touchmove",B,D),e.target?.removeEventListener("touchend",$)}t.onScopeDispose(()=>{window.removeEventListener("touchmove",B),window.removeEventListener("mousemove",B),window.removeEventListener("mouseup",F)})
...
return{value:t,position:M(t)}}):[]:[]),O=t.computed(()=>L.value.some(({label:e})=>!!e)),j={activeThumbRef:_,color:t.toRef(()=>e.color),decimals:g,disabled:i.isDisabled,direction:t.toRef(()=>e.direction),elevation:t.toRef(()=>e.elevation),hasLabels:O,isReversed:u,indexFromEnd:d,min:v,max:p,mousePressed:C,noKeyboard:t.toRef(()=>e.noKeyboard),numTicks:w,onSliderMousedown:function(e){0===e.button&&(e.preventDefault(),T(e),window.addEventListener("mousemove",B,D),window.addEventListener("mouseup",F,{passive:!1}))},onSliderTouchstart:function(e){T(e),window.addEventListener("touchmove",B,D),e.target?.addEventListener("touchend",$,{passive:!1})},parsedTicks:L,parseMouseMove:P,position:M,readonly:i.isReadonly,rounded:t.toRef(()=>e.rounded),roundValue:h,showTicks:z,startOffset:E,step:f,thumbSize:y,thumbColor:S,thumbLabelColor:k,thumbLabel:t.toRef(()=>e.thumbLabel),ticks:t.toRef(()=>e.ticks),tickSize:b,trackColor:x,trackContainerRef:I,trackFillColor:N,trackSize:V,vertical:c}
return t.provide(qc,j),j},ed=rt({focused:Boolean,max:{type:Number,required:!0},min:{type:Number,required:!0},modelValue:{type:Number,required:!0},position:{type:Number,required:!0},ripple:{type:[Boolean,Object],default:!0},name:String,noKeyboard:Boolean,...it()},"VSliderThumb"),td=ft()({name:"VSliderThumb",directives:{vRipple:An},props:ed(),emits:{"update:modelValue":e=>!0},setup(e,{slots:a,emit:l}){const n=t.inject(qc),{isRtl:o,rtlClasses:r}=ca()
...
t?.releasePointerCapture(e.pointerId),e.preventDefault(),o()}function T(e){const t=e.currentTarget
t?.setPointerCapture(e.pointerId),e.preventDefault(),e.stopPropagation(),n("up")}function D(e){const t=e.currentTarget
...
if(h=C(p-e*g.value,m),S(h))return _(h)}}function A(t){e.disabled||(t.preventDefault(),window.addEventListener("mousemove",P),window.addEventListener("touchmove",P),window.addEventListener("mouseup",T),window.addEventListener("touchend",T),i.value=null,s.value=null,r.value=!0,P(t))}function T(e){e.stopPropagation(),D(),r.value=!1,null!==s.value&&S(s.value)&&a("change",s.value)}function D(){window.removeEventListener("mousemove",P),window.removeEventListener("touchmove",P),window.removeEventListener("mouseup",T),window.removeEventListener("touchend",T)}t.watch(()=>e.modelValue,e=>{o.value=e}),t.onScopeDispose(D),It(()=>t.createElementVNode("div",{class:t.normalizeClass([{"v-time-picker-clock":!0,"v-time-picker-clock--indeterminate":null==e.modelValue,"v-time-picker-clock--readonly":e.readonly}]),onMousedown:A,onTouchstart:A,onWheel:k,ref:l},[t.createElementVNode("div",{class:"v-time-picker-clock__inner",ref:n},[t.createElementVNode("div",{class:t.normalizeClass([{"v-time-picker-clock__hand":!0,"v-time-picker-clock__hand--inner":x(e.modelValue)},c.value]),style:t.normalizeStyle([{transform:`rotate(${e.rotate+g.value*(y.value-e.min)}deg) scaleY(${N(y.value)})`},d.value])},null),V.value.map(a=>{const l=a===y.value
...
if(a&&e in a)return a[e]}Lg.version="4.0.4"
const jg=(e={})=>Lg({components:Rg,directives:Mg,...e}),Hg="4.0.4"
